...
const gatewayIcons: Record<string, typeof CreditCard> = { paystack: CreditCard, mpesa: Smartphone, bank: Building2 };
...
const SuperAdminPaymentGateway = () => {
  ...
  const { data: gateways = [], isLoading } = useQuery({
    ...
    queryFn: async () => { return await api.admin.paymentGateways.list(); },
  });
  ...
  const updateGateway = (id: string, updates: Record<string, any>) => {
    ...
    setLocalGateways(current.map((g: any) => g.id === id ? { ...g, ...updates } : g));
  };

  const updateGatewayConfig = (id: string, configKey: string, value: string) => {
    const current = localGateways.length > 0 ? localGateways : gateways;
    setLocalGateways(current.map((g: any) => {
      if (g.id !== id) return g;
  ...
  const handleSave = async () => {
    ...
    try {
      for (const gw of displayGateways) {
        await api.admin.paymentGateways.update(gw.id, { is_enabled: gw.is_enabled, mode: gw.mode, config: gw.config });
      }
  ...
  return (
    <div className="space-y-6">
      <div className="section-card flex items-start gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 shrink-0"><CreditCard size={18} className="text-primary" /></div>
        <div><p className="text-sm font-semibold text-foreground">Payment Gateway Configuration</p><p className="text-xs text-muted-foreground">Manage API keys, webhooks, and enable/disable payment providers.</p></div>
      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-[9px] font-semibold uppercase px-1.5 py-0.5 rounded ${gw.is_enabled ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>{gw.is_enabled ? "Enabled" : "Disabled"}</span>
                    <span className={`text-[9px] font-semibold uppercase px-1.5 py-0.5 rounded ${gw.mode === "production" ? "bg-destructive/10 text-destructive" : "bg-warning/10 text-warning"}`}>{gw.mode}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => updateGateway(gw.id, { mode: gw.mode === "sandbox" ? "production" : "sandbox" })} className="text-[10px] text-primary font-medium underline">Switch to {gw.mode === "sandbox" ? "Production" : "Sandbox"}</button>
                <button onClick={() => updateGateway(gw.id, { is_enabled: !gw.is_enabled })}>{gw.is_enabled ? <ToggleRight size={28} className="text-success" /> : <ToggleLeft size={28} className="text-muted-foreground" />}</button>
              </div>
            </div>
            {gw.is_enabled && (
                ...
                {["api_key", "secret_key", "webhook_url", "callback_url"].map((key) => (
                  <div key={key}>
                    ...
                    <div className="flex gap-2">
                      <input className="input-field flex-1 font-mono text-xs" value={showSecrets[`${gw.id}_${key}`] ? (config[key] || "") : maskKey(config[key] || "")} onChange={(e) => updateGatewayConfig(gw.id, key, e.target.value)} />
                      <button onClick={() => setShowSecrets(prev => ({ ...prev, [`${gw.id}_${key}`]: !prev[`${gw.id}_${key}`] }))} className="flex h-10 w-10 items-center justify-center rounded-xl border border-border hover:bg-secondary transition-colors">
