...
import {
  ...
  DollarSign, Settings, Crown, Loader2
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-card border-r border-border flex flex-col transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between px-5 py-5 border-b border-border">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary"><Wallet size={18} className="text-primary-foreground" /></div>
            <div>
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden back-btn h-8 w-8"><X size={16} /></button>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-[10px] font-semibold uppercase tracking-wider flex items-center gap-1 ${isSuperAdmin ? "text-destructive" : "text-primary"}`}>
                {isSuperAdmin && <Crown size={10} />}{isSuperAdmin ? "Super Admin" : "Admin"}
              </p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-3">
            ...
            {adminNavItems.map((item) => {
              ...
              return (
                <button key={item.path} onClick={() => { navigate(item.path); setSidebarOpen(false); }}
                  className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}>
                  <item.icon size={18} /><span>{item.label}</span>{isActive && <ChevronRight size={14} className="ml-auto" />}
                </button>
              );
            })}
          </div>
          ...
          {isSuperAdmin && superAdminNavItems.length > 0 && (
            <div className="mt-4 pt-4 border-t border-border space-y-0.5">
              <p className="text-[9px] font-semibold uppercase text-destructive px-3 mb-2 tracking-wider flex items-center gap-1"><Crown size={9} /> Super Admin</p>
              {superAdminNavItems.map((item) => {
                const isActive = location.pathname === item.path;
                return (
                  <button key={item.path} onClick={() => { navigate(item.path); setSidebarOpen(false); }}
                    className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${isActive ? "bg-destructive/10 text-destructive" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}>
                    <item.icon size={18} /><span>{item.label}</span>{isActive && <ChevronRight size={14} className="ml-auto" />}
                  </button>
                );
              })}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center justify-between px-5 py-4 border-b border-border bg-card">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="back-btn lg:hidden"><Menu size={18} /></button>
            <h1 className="text-lg font-bold text-foreground">{allPaths.find(i => i.path === location.pathname)?.label || "Admin"}</h1>
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            {isSuperAdmin && <Crown size={12} className="text-destructive" />}
            <div className="h-2 w-2 rounded-full bg-success" /> Online
          </div>
        </header>
