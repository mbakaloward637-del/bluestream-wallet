...
const AdminWithdrawals = () => {
  ...
  const { data: withdrawals = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      return await api.admin.withdrawals();
    },
  });

  const pending = withdrawals.filter((w: any) => w.status === "pending");

  const handleAction = async (id: string, status: "approved" | "rejected", userName: string) => {
    try {
      ...
      {pending.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {pending.map((w: any) => {
              const name = w.user_name || w.profiles?.first_name ? `${w.profiles?.first_name || ""} ${w.profiles?.last_name || ""}` : "Unknown";
              return (
                <div key={w.id} className="section-card">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="text-sm font-semibold text-foreground">{name}</p>
                      <p className="text-xs text-muted-foreground">{w.wallet_number || ""}</p>
                    </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleAction(w.id, "approved", name)}
                    ...
                    <button onClick={() => handleAction(w.id, "rejected", name)}
      ...
      <div>
        ...
        <div className="section-card p-0 divide-y divide-border">
          {withdrawals.length === 0 ? (
            ...
            const name = w.user_name || (w.profiles ? `${w.profiles.first_name} ${w.profiles.last_name}` : "Unknown");
            return (
